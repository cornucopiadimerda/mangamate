  ...
  return (
    <div className="flex flex-col" style={{ background: '#0C0C0E', minHeight: '100dvh' }}>
      ...
      <div className="px-5 pt-14 pb-4">
        <p style={{ fontSize: 13, fontWeight: 500, color: '#8A8A8E', letterSpacing: 0.3 }}>
      ...
      <div className="flex gap-3 px-5 pb-5">
        <StatCard value={totalVolumes} label="Volumi" color="#E91E8C" />
      ...
      <div className="px-5 pb-5">
        <Link href="/scan" className="block tap-scale">
          <div
            ...
            style={{ background: 'linear-gradient(135deg, #E91E8C 0%, #B5179E 100%)' }}
          >
            <div
                ...
                >
                  {series.coverUrl ? (
                    <div style={{ width: 48, height: 68, borderRadius: 6, overflow: 'hidden', flexShrink: 0, position: 'relative' }}>
                      <Image src={series.coverUrl} alt={series.title} fill style={{ objectFit: 'cover' }} sizes="48px" onError={() => {}} />
                    </div>
                  ) : (
                    <CoverPlaceholder seriesId={series.id} title={series.title} width={48} height={68} />
                  )}
                  <div className="flex-1 min-w-0">
                    <p style={{ fontSize: 15, fontWeight: 700, color: '#FFFFFF' }}>{series.title}</p>
                    <p style={{ fontSize: 12, color: '#8A8A8E', marginTop: 2 }}>{series.publisher}</p>
                    <div className="flex items-center gap-2 mt-2">
                      <div style={{ flex: 1, height: 3, background: '#1E1E22', borderRadius: 99 }}>
                        <div style={{ height: '100%', width: `${series.completionPercent}%`, background: '#E91E8C', borderRadius: 99 }} />
                      </div>
                      <span style={{ fontSize: 11, fontWeight: 700, color: '#E91E8C', flexShrink: 0 }}>
                  ...
                  <div className="flex-shrink-0 flex items-center justify-center rounded-xl"
                    ...
                    <div>
                      <p style={{ fontSize: 11, fontWeight: 700, color: '#E91E8C', textAlign: 'center' }}>{series.missingVolumes.length}</p>
                      <p style={{ fontSize: 9, color: '#8A8A8E', textAlign: 'center' }}>mancanti</p>
                    </div>
                  </div>
                </div>
              </Link>
        ...
        <section className="pb-5">
          ...
          <div className="px-5 flex flex-col gap-2">
            {wishlist.slice(0, 3).map((item) => (
              <div key={item.id} className="flex items-center gap-3 p-3 rounded-xl" style={{ background: '#141416' }}>
                <div className="flex items-center justify-center rounded-lg"
                  style={{ width: 36, height: 36, background: 'rgba(233,30,140,0.15)' }}>
                  <Heart size={16} color="#E91E8C" />
                </div>
                <div className="flex-1 min-w-0">
                  <p style={{ fontSize: 13, fontWeight: 600, color: '#FFFFFF' }}>{item.seriesTitle}</p>
                  <p style={{ fontSize: 11, color: '#8A8A8E' }}>Volume {item.volumeNumber}</p>
                </div>
              </div>
            ))}
            {wishlist.length > 3 && (
              <Link href="/wishlist">
                <p style={{ fontSize: 13, color: '#E91E8C', fontWeight: 600, textAlign: 'center', padding: '8px 0' }}>
      ...
      <div className="px-5 pb-5">
        <div className="flex items-start gap-3 p-4 rounded-2xl"
          ...
          <div>
            ...
            <p style={{ fontSize: 12, color: '#8A8A8E', marginTop: 3, lineHeight: 1.5 }}>
...
function StatCard({ value, label, color }: { value: string | number; label: string; color: string }) {
  return (
    <div className="flex-1 rounded-2xl p-3.5" style={{ background: '#141416', border: '1px solid #1E1E22' }}>
      <p style={{ fontSize: 22, fontWeight: 800, color, lineHeight: 1 }}>{value}</p>
      <p style={{ fontSize: 11, color: '#8A8A8E', marginTop: 4, lineHeight: 1.3 }}>{label}</p>
    </div>
  )
}
...
function SectionHeader({ title, href }: { title: string; href: string }) {
  return (
    <div className="flex items-center justify-between px-5 mb-3">
      ...
      <Link href={href}>
        <span style={{ fontSize: 13, color: '#E91E8C', fontWeight: 600 }}>Vedi tutti</span>
      </Link>
    </div>
  )
}
