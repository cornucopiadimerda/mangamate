...
export default function ProfilePage() {
  ...
  return (
    <div style={{ background: '#0C0C0E', minHeight: '100dvh', paddingBottom: 'calc(env(safe-area-inset-bottom) + 80px)' }}>
      ...
      <div className="px-5 pt-14 pb-6">
        <div className="flex items-start justify-between">
              ...
              {editingName ? (
                <div className="flex items-center gap-2 mt-1">
                  <input
                    ...
                    autoFocus
                    style={{ background: '#141416', border: '1px solid #E91E8C', borderRadius: 8, color: '#FFFFFF', fontSize: 13, padding: '4px 8px', outline: 'none', width: 160 }}
                  />
                </div>
              ) : (
                <button onClick={() => setEditingName(true)} className="flex items-center gap-1 mt-1">
                  <span style={{ fontSize: 13, color: '#8A8A8E' }}>{profile.tagname}</span>
                  <Edit3 size={11} color="#4A4A50" />
                </button>
              )}
              <p style={{ fontSize: 12, color: '#4A4A50', marginTop: 2 }}>{profile.bio}</p>
            </div>
          </div>
          <button onClick={shareStats} className="tap-scale flex items-center gap-1.5 px-3 py-2 rounded-xl"
            ...
            <Share2 size={15} color="#E91E8C" />
            <span style={{ fontSize: 12, fontWeight: 600, color: '#E91E8C' }}>Condividi</span>
          </button>
        </div>
      </div>
      ...
      {/* Stats grid */}
      <div className="px-5 pb-5">
        <h2 style={{ fontSize: 11, fontWeight: 700, color: '#8A8A8E', marginBottom: 12, textTransform: 'uppercase', letterSpacing: 0.5 }}>
          Statistiche collezione
        </h2>
        <div className="grid grid-cols-2 gap-3">
          <StatBlock icon={<BookOpen size={18} color="#E91E8C" />} value={totalVolumes} label="Volumi totali" color="#E91E8C" />
          ...
          <StatBlock icon={<span style={{ fontSize: 18 }}>📦</span>} value={totalMissing} label="Mancanti" color="#8A8A8E" />
          ...
          {MOCK_SERIES.map(series => {
            ...
            return (
              <div key={series.id} className="flex items-center gap-3 p-3.5 rounded-2xl"
                style={{ background: '#141416', border: isFav ? '1px solid rgba(233,30,140,0.3)' : '1px solid #1E1E22' }}>
                <div className="flex-shrink-0 rounded-lg overflow-hidden" style={{ width: 40, height: 56, background: '#0E0E11' }}>
                  {series.coverUrl ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center"
                      style={{ fontSize: 20, fontWeight: 900, color: '#3C3C44' }}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p style={{ fontSize: 11, color: '#8A8A8E' }}>{series.author}</p>
                  {collSeries && (
                    <p style={{ fontSize: 11, color: isFav ? '#E91E8C' : '#4A4A50', marginTop: 2 }}>
                ...
                <button onClick={() => toggleFavorite(series.id)} className="tap-scale flex-shrink-0"
                  style={{ width: 36, height: 36, background: isFav ? 'rgba(233,30,140,0.15)' : '#1E1E22', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Heart size={16} color="#E91E8C" fill={isFav ? '#E91E8C' : 'none'} />
                </button>
              </div>
            )
          })}
...
function StatBlock({ icon, value, label, color }: { icon: React.ReactNode; value: string | number; label: string; color: string }) {
  return (
    <div className="p-4 rounded-2xl" style={{ background: '#141416', border: '1px solid #1E1E22' }}>
      <div className="flex items-center gap-2 mb-2">{icon}</div>
      <p style={{ fontSize: 22, fontWeight: 800, color, lineHeight: 1 }}>{value}</p>
      <p style={{ fontSize: 11, color: '#8A8A8E', marginTop: 4 }}>{label}</p>
    </div>
  )
}
