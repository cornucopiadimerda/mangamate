  ...
  if (!series) {
    return (
      <div className="flex items-center justify-center h-screen" style={{ background: '#080808' }}>
        <p style={{ color: '#8E8E93' }}>Serie non trovata</p>
      </div>
    )
  }
    ...
    <div style={{ background: '#080808', minHeight: '100dvh' }}>
      ...
      >
        ...
        <div className="relative px-5">
          <div className="flex items-start gap-4">
            ...
            >
              <span style={{ fontSize: 52, fontWeight: 900, color: colors.text, opacity: 0.5 }}>
                {series.title.charAt(0)}
              </span>
            </div>
            ...
            <div className="flex-1 min-w-0 pt-1">
              <h1 style={{ fontSize: 24, fontWeight: 800, color: '#FFFFFF', letterSpacing: -0.5, lineHeight: 1.1 }}>
                {series.title}
              </h1>
              <p style={{ fontSize: 14, color: colors.text, marginTop: 4, fontWeight: 600 }}>
                {series.author}
              </p>
              ...
              <div className="flex items-center gap-2 mt-3">
                <span
                  style={{
                    ...
                    background: `${statusColor[series.status]}22`,
                    padding: '3px 8px',
                    borderRadius: 99,
                  }}
                >
            ...
            <div className="flex items-center justify-between mb-2">
              <div>
                <span style={{ fontSize: 26, fontWeight: 800, color: '#FFFFFF' }}>
                  {series.ownedVolumes.length}
                </span>
                <span style={{ fontSize: 14, color: 'rgba(255,255,255,0.5)' }}>
                  /{series.totalVolumes} volumi
                </span>
              </div>
              <span
                style={{
                  fontSize: 20,
                  fontWeight: 800,
                  color: isComplete ? '#30D158' : colors.text,
                }}
              >
            ...
            <div style={{ height: 6, background: 'rgba(255,255,255,0.1)', borderRadius: 99 }}>
              <div
                style={{
                  ...
                  background: isComplete ? '#30D158' : colors.text,
                  borderRadius: 99,
      ...
      {series.description && (
        <div className="px-5 py-4">
          <p style={{ fontSize: 13, color: '#8E8E93', lineHeight: 1.6 }}>{series.description}</p>
        </div>
      )}
          ...
          {series.missingVolumes.length > 0 && (
            <Link href={`/wishlist?series=${seriesId}`}>
              <span style={{ fontSize: 13, color: '#FF3B30', fontWeight: 600 }}>
                {series.missingVolumes.length} mancanti
              </span>
            </Link>
          )}
        </div>
      ...
      <div className="px-5 pt-2 pb-4 flex gap-3">
        ...
        <Link href={`/wishlist?series=${seriesId}`} className="tap-scale">
          <div
            ...
            style={{ width: 50, height: 50, background: '#1C1C1E', border: '1px solid #2C2C2E' }}
          >
