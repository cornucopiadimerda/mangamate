    ...
    <div className="fixed inset-0 z-[100] flex items-end"
      ...
      <div className="w-full animate-fade-in"
        ...
        <div className="px-5 pb-2">
          ...
          <p style={{ fontSize: 13, color: '#8A8A8E', marginBottom: 20 }}>{title} — Vol. {volume}</p>
          <div className="flex flex-col gap-3">
            {options.map((opt) => (
              <a key={opt.name} href={opt.url} target="_blank" rel="noopener noreferrer"
                ...
                style={{ background: opt.primary ? '#E91E8C' : '#1E1E22', textDecoration: 'none' }}
                onClick={onClose}>
                <span style={{ fontSize: 26 }}>{opt.emoji}</span>
                <div className="flex-1">
                  <p style={{ fontSize: 15, fontWeight: 700, color: '#FFFFFF' }}>{opt.name}</p>
                  <p style={{ fontSize: 11, color: opt.primary ? 'rgba(255,255,255,0.7)' : '#8A8A8E' }}>{opt.subtitle}</p>
                </div>
                <span style={{ fontSize: 18, color: opt.primary ? 'rgba(255,255,255,0.6)' : '#3C3C44' }}>›</span>
              </a>
            ))}
          </div>
          {!AMAZON_TAG && (
            <p style={{ fontSize: 10, color: '#282830', marginTop: 16, textAlign: 'center', lineHeight: 1.5 }}>
